...
def execute_query_with_dataframe(request):
    try:
        ...
        logging.info(f"Metadata updated in delta_processed with date_run: {date_run}")
         ...
         # Call the second function and get its result
        second_result = second_execute_query_with_dataframe(request)
        third_result = third_execute_query_with_dataframe(request)
        fourth_result = fourth_execute_query_with_dataframe(request)
        ...
        result = {
            ...
            "second_function_result": second_result,
            "third_function_result": third_result,
            "fourth_function_result": fourth_result
        }
...
def second_execute_query_with_dataframe(request):
    try:
        # Initialize BigQuery and Cloud Storage clients
        bq_client = bigquery.Client(project=PROJECT_ID)
        storage_client = storage.Client(project=PROJECT_ID)
        ...
        def update_date_summary(bq_client):
            ...
            if not query_from_table:
                raise ValueError("No query found for query_id = 6")
        ...
        third_execute_query_with_dataframe(request)
        
        return {
            "message": "CSV files created successfully by AccrualTransaction",
            "total_rows": total_rows,
            "file_count": file_count
        }
...
def third_execute_query_with_dataframe(request):
    try:
        # Initialize BigQuery and Cloud Storage clients
        bq_client = bigquery.Client(project=PROJECT_ID)
        storage_client = storage.Client(project=PROJECT_ID)
        ...
        def update_date_summary(bq_client):
            ...
            if not query_from_table:
                raise ValueError("No query found for query_id = 4")
        ...
        return {
            "message": "CSV files created successfully by Timecardtransaction",
            "total_rows": total_rows,
            "file_count": file_count
        }
...
def fourth_execute_query_with_dataframe(request):
    try:
        # Initialize BigQuery and Cloud Storage clients
        bq_client = bigquery.Client(project=PROJECT_ID)
        storage_client = storage.Client(project=PROJECT_ID)
        ...
        def update_date_summary(bq_client):
            ...
            if not query_from_table:
                raise ValueError("No query found for query_id = 7")
        ...
        return {
            "message": "CSV files created successfully by ScehduleOpenShift",
            "total_rows": total_rows,
            "file_count": file_count
        }
